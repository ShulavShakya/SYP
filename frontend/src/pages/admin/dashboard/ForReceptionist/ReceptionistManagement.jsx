...
export default function ReceptionistManagement() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredReceptionists.length / recordsPerPage),
  );

  const paginatedReceptionists = useMemo(() => {
    const startIndex = (currentPage - 1) * recordsPerPage;
    return filteredReceptionists.slice(startIndex, startIndex + recordsPerPage);
  ...
  const handleDeleteReceptionist = async (receptionist) => {
    ...
    try {
      ...
      setReceptionists((prev) => {
        const updated = prev.filter((item) => item.id !== receptionist.id);
        ...
        const updatedFiltered =
          selectedShift === "All"
            ...
            : updated.filter(
                (item) =>
                  formatShiftLabel(item.shift).toLowerCase() ===
                  selectedShift.toLowerCase(),
              );

        const newTotalPages = Math.max(
          1,
          Math.ceil(updatedFiltered.length / recordsPerPage),
        );
    ...
    <>
      ...
      {selectedReceptionist && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) closeReceptionistModal();
          }}
        >
          <div className="w-full max-w-3xl rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  View complete information for this receptionist
                </p>
              </div>
            ...
            <div className="max-h-[85vh] space-y-6 overflow-y-auto p-6">
              <div className="mb-2 flex items-center gap-4">
                {imagePreview ? (
                  ...
                  <div className="flex h-16 w-16 items-center justify-center rounded-full bg-primary/10 text-lg font-bold text-primary">
                    {getInitials(selectedReceptionist.name)}
                  </div>
                )}
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {selectedReceptionist.Receptionist_id || "-"}
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                ...
                <div className="space-y-4">
                  ...
                  />
                  <InfoRow label="Gender" value={selectedReceptionist.gender} />
                  <InfoRow
                    label="Date of Birth"
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                ...
                <div className="space-y-4">
                  ...
                  <InfoRow
                    label="Created At"
                    value={selectedReceptionist.created_at}
                  />
                </div>
              </div>
            </div>
